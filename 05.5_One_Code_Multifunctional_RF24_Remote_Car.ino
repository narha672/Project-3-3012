...
void updateAutomaticTrackingLine() {
  ...
  switch (trackingSensorVal) {
    case 0: // 000: Line lost, search for the line or move forward slightly to find it
      motorRun(TK_FORWARD_SPEED_LOW, TK_FORWARD_SPEED_LOW);
      break;
    case 1: // 001: Should turn right
      motorRun(TK_TURN_SPEED_LV1, TK_TURN_SPEED_LV4); // Adjusted for right turn
      break;
    case 4: // 100: Should turn left
      motorRun(TK_TURN_SPEED_LV4, TK_TURN_SPEED_LV1); // Adjusted for left turn
      break;
    case 2: // 010: Center, move forward
      motorRun(TK_FORWARD_SPEED, TK_FORWARD_SPEED);
      break;
    case 3: // 011: Sharp right
      motorRun(TK_TURN_SPEED_LV2, TK_TURN_SPEED_LV4);
      break;
    case 6: // 110: Sharp left
      motorRun(TK_TURN_SPEED_LV4, TK_TURN_SPEED_LV2);
      break;
    case 7: // 111: Line is directly underneath, stop or adjust based on your design
      motorRun(TK_STOP_SPEED, TK_STOP_SPEED);
      break;
    default: // Other cases
      motorRun(TK_STOP_SPEED, TK_STOP_SPEED); // Stop as a default action
      break;
  }
}
...
// void updateCarActionByNrfRemote() {
//   int x = nrfDataRead[JOYSTICK_X] - 512; // Centered at 512
//   int y = nrfDataRead[JOYSTICK_Y] - 512; // Centered at 512
//   int pwmL, pwmR;

//   // Map Y value (forward/backward) directly to motor speeds
//   // Note: May need to adjust multiplier for speed scaling based on your setup
//   pwmL = pwmR = y;

//   // Adjust left and right speeds based on X value for turning
//   if (x > 0) { // Turning right
//     pwmL = pwmL + x / 2; // Increase left motor speed
//     pwmR = pwmR - x / 2; // Decrease right motor speed
//   } else { // Turning left
//     pwmL = pwmL + x / 2; // Decrease left motor speed (x is negative)
//     pwmR = pwmR - x / 2; // Increase right motor speed
//   }

//   // Apply the calculated speeds to the motors
//   motorRun(pwmL, pwmR);

//   // Buzzer control remains the same
//   if (nrfDataRead[JOYSTICK_Z] == 0) {
//     setBuzzer(true);
//   } else {
//     setBuzzer(false);
...
void updateCarActionByNrfRemote() {
  int x = nrfDataRead[JOYSTICK_X] - 512; // Centered at 512
  // Invert Y-axis for forward/backward direction correction
  int y = 512 - nrfDataRead[JOYSTICK_Y]; // Invert by subtracting from midpoint max
  ...
  // Adjust for turning based on X value
  // Increase/decrease motor speed for turning
  if (x > 0) { // Turning right
    pwmL += x / 2;
    pwmR -= x / 2;
  } else { // Turning left
    pwmL += x / 2; // x is negative, so it decreases pwmL
    pwmR -= x / 2; // x is negative, increases pwmR
  }

  // Apply calculated speeds to motors, accounting for inversion if necessary
  motorRun(pwmL, pwmR);

  // Control buzzer based on joystick button
  if (nrfDataRead[JOYSTICK_Z] == 0) {
    setBuzzer(true);
  } else {
    setBuzzer(false);
